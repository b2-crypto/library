...
import { DATA_ZOOM_CHART_START, MIN_CHART_ITEMS_COUNT, } from '../../helpers/constants';
import { hexToRgbA } from '../../helpers/common';
import { getChartMarklineOptions } from '../../helpers/getChartMarklineOptions';
import { ChartLineType, AxisType, ChartDatasetType, } from '../../types/chartTypes';
// register extensions
echarts.use([EchartsLineChart]);
export const LineChart = ({ data, backgroundColor, fillShadowGradientFrom, fillShadowGradientTo, lineColor, fillInside = true, isScrollDisabled = false, showYLabels = false, showXLabels = true, showXTick = true, showXMinLabel = false, typeYLabels = AxisType.value, typeAxisLine = ChartLineType.solid, typeGridLine = ChartLineType.solid, withVerticalGrid = true, withHorizontalGrid = true, markLineValue, minHeight = 200, ...chartProps }) => {
    const theme = useTheme();
    const { colors, spacing, textVariants: { chartAxisText }, } = theme;
    ...
    const yAxisProps = {
        ...
        axisLabel: {
            ...
            show: showYLabels,
            margin: 5,
            formatter: typeYLabels === AxisType.time &&
                function (value) {
    ...
    const dataZoom = [
        {
            ...
            type: 'inside',
            start: dataXLabel.length > MIN_CHART_ITEMS_COUNT && !isScrollDisabled
