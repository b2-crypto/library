...
const ExpandedSearch = memo(({ onCollapse, onExpand, isExpanded, ...inputProps }) => {
    ...
    const prefix = useMemo(() => {
        ...
        return (<IconComponent width={16} height={16} onTouchEnd={handleIconPress}/>);
    ...
    return (<Input onFocus={handleFocus} size="small" placeholder={translate('search')} boxProps={{
            paddingLeft: 'sm',
            flex: isExpanded ? 1 : 0,
            maxWidth: isExpanded ? 'auto' : 43,
        }} prefix={prefix} autoCapitalize="none" ref={input => {
            if (input) {
                inputRef.current = input;
            }
        }} {...inputProps}/>);
});
export const ExpandableSearchInput = (props) => {
    ...
    return (<Animated.View style={[styles.container, { flex: interpolatedFlex }]}>
      <ExpandedSearch onCollapse={handleCollapse} isExpanded={isExpanded} onExpand={handleExpand} {...inputProps}/>
    </Animated.View>);
};
