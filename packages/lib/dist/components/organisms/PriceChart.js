...
export const PriceChart = memo(({ data, color, symbol }) => {
    ...
    return (<Box position="relative">
      <Box alignItems="flex-end" position="absolute" right={0} top={0} zIndex={1} elevation={1}>
        <Box alignSelf="flex-end" pr="s" style={{
      ...
      <Box alignItems="flex-end" position="absolute" right={0} bottom={27} zIndex={1} elevation={1}>
        <Box alignSelf="flex-end" pr="s" style={{
            backgroundColor: hexToRgbA(theme.colors.background2, 0.6),
        }}>
          <Text variant="chartAxisText" color="textSecondary">
      ...
      <LineChart height={120} data={chartData} isScrollDisabled showXLabels showXTick={false} fillShadowGradientFrom={hexToRgbA(chartColor, 0.3)} fillShadowGradientTo={hexToRgbA(chartColor, 0.05)} lineColor={chartColor} typeAxisLine={ChartLineType.dashed} withVerticalGrid={false} withHorizontalGrid={false}/>
    </Box>);
});
...
export const PriceChartWidget = memo(({ instrumentId, ...props }) => {
    const theme = useTheme();
    const isFocused = useIsFocused();
    const { data: priceChartData, isLoading } = useGetTickerHistoryQuery(isFocused
        ? {
            instrumentId,
    ...
    if (isLoading) {
        return (<ActivityIndicator size="large" style={styles.spinner} color={theme.colors.brandSolid}/>);
    }
    return (<PriceChart {...props} data={isFocused && priceChartData ? priceChartData : []}/>);
});
