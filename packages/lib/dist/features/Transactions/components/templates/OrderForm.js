...
import { InstrumentFieldWidget, OrderFormButton, OrderFormHeader, OrderTypeField, } from '../molecules';
import { MarginOrderSummaryWidget, OrderFormBodyWidget, OrderSummaryWidget, } from '../organisms';
import { useMarginAccount, useMarginEnabled } from '../../../Margin';
import { OrderFormValidationContext, } from '../../hooks/useOrderFormValidationContext';
import { OrderConfirm } from './OrderConfirm';
const BUTTON_CONTAINER_HEIGHT = 130;
export const OrderForm = ({ onClose, initialValues }) => {
    ...
    const form = useOrderForm({ accountId: currentAccount?.AccountId || 0, ...initialValues }, validationContext);
    ...
    useEffect(() => {
        ...
        const subscription = form.watch((values, { name }) => {
            if (name && fieldsToWatch.includes(name)) {
                const key = name;
    ...
    return (<OrderFormValidationContext.Provider value={useMemo(() => ({ value: validationContext, set: setValidationContext }), [validationContext, setValidationContext])}>
      <FormProvider {...form}>
        <Box position="relative" height="100%" style={styles.scrollViewContainer}>
          <ScrollView 
    ...
    keyboardShouldPersistTaps="always">
            ...
            <OrderTypeField />
            {error && (<SubmissionError error={isApexError(error)
                ? error.message
                : translate('common.unknownError')} onDismiss={reset}/>)}
            <OrderFormBodyWidget />
            <DashedBox topDash padding="m">
              {isMarginEnabled &&
            marginAccount?.AccountId === form.getValues().accountId ? (<MarginOrderSummaryWidget />) : (<OrderSummaryWidget />)}
            </DashedBox>
          </ScrollView>
          <Box style={styles.buttonContainer} shadowOffset={{ width: 0, height: -4 }} shadowRadius={14} shadowColor="black" shadowOpacity={0.1} elevation={7} backgroundColor="background1" position="absolute" left={0} right={0} padding="m" borderTopWidth={1} borderTopColor="border3">
            <OrderFormButton onPress={showModal}/>
          </Box>
        </Box>

        <ModalBlurBg isVisible={modalVisible} onBackdropPress={hideModal} style={styles.modal}>
          {!!modalVisible && (<OrderConfirm values={form.getValues()} onClose={hideModal} submitting={isLoading} onSubmit={form.handleSubmit(onSubmit)}/>)}
        </ModalBlurBg>
      </FormProvider>
