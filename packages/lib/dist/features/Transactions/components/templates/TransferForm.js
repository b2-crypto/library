...
export const TransferForm = ({ productId, onClose, onSuccess }) => {
    const accountsPositions = useAllAccountsPositions();
    const { data: products } = useGetProductsQuery();
    const [isSubmitting, setIsSubmitting] = React.useState(false);
    const balances = React.useMemo(() => mapValues(accountsPositions, positions => positions.reduce((acc, p) => {
    ...
    const onSubmit = React.useCallback(async (values) => {
        ...
        try {
            ...
            Toast.show({
                ...
                text2: formatCurrency(Number(values.amount), product?.Product, product?.DecimalPlaces, false),
            });
            onClose();
            onSuccess?.();
        }
    ...
    return (<FormProvider {...form}>
      <ScrollView>
        ...
        <Box padding="m">
          <Button label={translate('transactions.transferButton')} disabled={!form.formState.isValid || isSubmitting} loading={isSubmitting} onPress={form.handleSubmit(onSubmit)}/>
        </Box>
      </ScrollView>
    </FormProvider>);
};
