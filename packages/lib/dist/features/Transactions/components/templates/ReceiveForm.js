...
import { useCryptoAssets, useReceiveForm, useRequestTransferFunds, } from '../../hooks';
import { isApexError } from '../../../../types/apiResponses';
import { useModalControl } from '../../../../hooks';
import { useUserInfo } from '../../../../hooks';
import { testID } from '../../../../helpers/testId';
import { ReceiveConfirm } from './ReceiveConfirm';
export const ReceiveForm = ({ productId, onClose }) => {
    ...
    keyboardShouldPersistTaps="always">
        <ReceiveFormHeader onClose={onClose}>
          <AssetField assets={assets} label={translate('receive')} isLoading={isAssetsLoading}/>
        </ReceiveFormHeader>
        {assetId && asset ? (<>
            <ReceiveTypeTabs />
            <Box padding="m">
              {type === 'deposit' ? (<ReceiveQRWidget />) : (<>
                  <TransferCryptoFormBody asset={asset}/>
                  {!!error && isApexError(error) && (<Box mt="m">
                      <SubmissionError error={error.message}/>
                    </Box>)}
                  <Button disabled={!formState.isValid} label={translate('transactions.requestAssets')} onPress={showModal} mt="m" accessibilityLabel="Request" {...testID('submit')}/>
                </>)}
            </Box>
          </>) : (<SelectAssetMessage message={translate('transactions.selectAsset.receive')}/>)}
      </ScrollView>

      <ModalBlurBg isVisible={modalShown} onBackdropPress={hideModal} style={styles.modal}>
        {modalShown && (<ReceiveConfirm values={values} asset={asset} onClose={hideModal} submitting={isLoading} onSubmit={handleSubmit(onSubmit)}/>)}
      </ModalBlurBg>
    </FormProvider>);
};
