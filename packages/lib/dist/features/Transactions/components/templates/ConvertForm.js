...
import { useGetInstrumentWithLevel1, useModalControl, useWallets, useAppSelector, } from '../../../../hooks';
import { getLogger } from '../../../../services';
import { useConvertForm, useConvertSubmit } from '../../hooks';
import { useGetProductsQuery, useGetUserInfoQuery, } from '../../../../services/apexApi';
import { isApexError } from '../../../../types/apiResponses';
import { Box } from '../../../../components/atoms';
import { Button, ModalBlurBg, SubmissionError, } from '../../../../components/molecules';
...
export const ConvertForm = ({ onClose }) => {
    const user = useAppSelector(selectUser);
    const { data: userInfo } = useGetUserInfoQuery(user?.UserId ? { userId: user.UserId } : skipToken);
    const { data: instruments } = useGetInstrumentWithLevel1();
    const { data: allProducts } = useGetProductsQuery();
    const { data: wallets } = useWallets({});
    const instrumentProductIds = useMemo(() => Array.from(instruments.reduce((acc, i) => acc.add(i.Product1).add(i.Product2), new Set())), [instruments]);
    ...
    const products = useMemo(() => allProducts
        ?.filter(p => (p.ProductType === 'NationalCurrency' ||
        ...
        return {
            ...
            type: p.ProductType === 'NationalCurrency'
                ? 'fiat'
                : 'crypto',
        ...
        .sort((a, b) => a.symbol.localeCompare(b.symbol)) || [], [allProducts, wallets, instrumentProductIds]);
    return (<FormProvider {...form}>
      <ScrollView>
        <ConvertFormHeader onClose={onClose}/>
        <Box mt="s">
          <ConvertFormBodyWidget instruments={instruments} products={products}/>
        </Box>
        <Box mt="l" paddingHorizontal="xl" paddingVertical="m" borderTopWidth={1} borderColor="border3">
          <ConvertSummaryWidget />
          {error && (<Box mt="m">
              <SubmissionError error={isApexError(error)
                ? error.message
                : translate('common.unknownError')}/>
            </Box>)}
        </Box>
        <Box padding="m">
          <Button label={translate('transactions.convert_button')} disabled={!form.formState.isValid} onPress={onConvertPress}/>
        </Box>
      </ScrollView>

      <ModalBlurBg isVisible={modalVisible} onBackdropPress={hideModal} style={styles.modal}>
        {modalVisible && (<ConvertConfirm values={form.getValues()} onClose={hideModal} submitting={isLoading} onSubmit={form.handleSubmit(onSubmit)}/>)}
      </ModalBlurBg>
    </FormProvider>);
};
