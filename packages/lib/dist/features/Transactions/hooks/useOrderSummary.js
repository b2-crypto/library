...
import { useGetInstrumentsQuery, useGetL2SnapshotQuery, useGetLevel1SummaryQuery, useGetOrderFeeQuery, useGetProductsQuery, useGetUserInfoQuery, } from '../../../services/apexApi';
...
export function useOrderSummary(values) {
    ...
    const { data: userInfo } = useGetUserInfoQuery(user ? { userId: user.UserId } : skipToken);
    const { instrument } = useGetInstrumentsQuery(hasSummary ? undefined : skipToken, {
    ...
    //#region MarketPrice
    const { level1 } = useGetLevel1SummaryQuery(hasSummary ? undefined : skipToken, {
        pollingInterval: QUERY_POLLING_INTERVAL_MS,
        selectFromResult: ({ data }) => ({
            level1: data?.find(l => l.InstrumentId === values.instrumentId),
        }),
    });
    const marketItem = hasSummary && level1 && instrument
        ? {
            amount: formatNumber(values.op === 'buy' ? level1.BestOffer : level1.BestBid, increment2digits(instrument.PriceIncrement), true),
    ...
    const { data: feeInfo } = useGetOrderFeeQuery(userInfo?.AccountId &&
        values.instrumentId &&
        values.type &&
        amountForFee &&
        instrument
    ...
    const feeItem = hasSummary && orderFee
        ? {
    ...
    //#endregion
    //#region Total Price
    const { totalPrice: marketTotalPrice } = useGetL2SnapshotQuery(values.instrumentId && values.type === 'Market' && !marketByValue
        ? { instrumentId: values.instrumentId }
        : skipToken, {
        pollingInterval: QUERY_POLLING_INTERVAL_MS,
        selectFromResult: ({ data }) => {
            const orders = data
                ...
                .sort((a, b) => values.op === 'buy' ? a.price - b.price : b.price - a.price) || [];
            let totalPrice = 0, leftQuantity = quantity, i = 0;
    ...
    const totalPriceItem = hasSummary && instrument
        ? {
            amount: formatNumber(totalPrice, increment2digits(instrument.PriceIncrement), true),
            symbol: instrument.Product2Symbol,
        }
        : null;
    //#endregion
    //#region Net
    const netItem = hasSummary && instrument
        ? {
            amount: formatNumber(values.op === 'buy'
                ? quantity -
                    (feeInfo?.ProductId === instrument.Product1 ? orderFee : 0)
                : totalPrice -
                    (feeInfo?.ProductId === instrument.Product2 ? orderFee : 0), increment2digits(values.op === 'buy'
                ? instrument.QuantityIncrement
                : instrument.PriceIncrement), true),
            symbol: values.op === 'buy'
