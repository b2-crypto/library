...
export function useTransferBody() {
    const { data: marginProductConfigs, isLoading: isMarginProductConfigsLoading, } = useMarginProductConfig();
    const { data: accounts, isLoading: isAccountsLoading } = useGetUserAccountsQuery();
    const { currentAccount } = useUserCurrentAccount();
    const initialValue = useMemo(() => ({ AccountId: 0, AccountName: 'Select account' }), []);
    const defaultFromAccount = accounts?.find(account => account.AccountId === currentAccount?.AccountId) || initialValue;
    ...
    const defaultToAccount = useMemo(() => {
        if (currentAccount?.AccountType === 'Margin') {
            return (accounts?.filter(account => account.AccountType === 'Asset')[0] ||
                initialValue);
        }
        return (accounts?.filter(account => account.AccountType === 'Margin')[0] ||
            initialValue);
    }, [accounts, currentAccount?.AccountType, initialValue]);
    const { data: rawProducts, isLoading: isProductsLoading } = useGetProductsQuery();
    const { watch, trigger } = useFormContext();
    const fromAccountId = watch('fromAccountId');
    const toAccountId = watch('toAccountId');
    const defaultProductId = watch('defaultProductId');
    const { data: positions, isLoading: isPositionsLoading } = useGetAccountPositionsQuery(fromAccountId ? { accountId: fromAccountId } : skipToken);
    // Re-validate `toAccountId` whenever `fromAccountId` changes (to check for equal values)
    useEffect(() => {
        trigger('toAccountId');
    }, [trigger, fromAccountId]);
    const toAccountIsMargin = accounts?.find(account => account.AccountId === toAccountId)
        ?.AccountType === 'Margin';
    ...
    const products = useMemo(() => {
        const productsList = rawProducts
            ?.map(p => {
            const marginProductConfig = marginProductConfigs?.find(config => config.ProductId === p.ProductId);
            const position = positions?.find(pos => pos.ProductId === p.ProductId);
            return {
                id: p.ProductId,
                symbol: p.Product,
