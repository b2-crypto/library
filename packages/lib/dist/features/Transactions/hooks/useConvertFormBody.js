...
export function useConvertFormBody({ instruments, products, }) {
    ...
    const defaultToProduct = useMemo(() => {
        const filtered = products.filter(p => !fromProduct ||
            instruments.some(i => (i.Product1 === fromProduct && i.Product2 === p.id) ||
                (i.Product2 === fromProduct && i.Product1 === p.id)));
        return (filtered.find(p => p.symbol === 'BTC')?.id ||
    ...
    const fromProducts = useMemo(() => products.filter(p => instruments.some(i => (i.Product1 === toProductId && i.Product2 === p.id) ||
        (i.Product2 === toProductId && i.Product1 === p.id))), [products, instruments, toProductId]);
    ...
    const instrument = useMemo(() => instruments.find(i => (i.Product1 === fromProductId && i.Product2 === toProductId) ||
        (i.Product1 === toProductId && i.Product2 === fromProductId)), [instruments, fromProductId, toProductId]);
    //#endregion
    //#region Input anchoring and recalculate value for unachored field
    const setAnchored = useCallback((nextState) => setValue('anchored', nextState), [setValue]);
    const isBuy = instrument?.Product1 === fromProduct;
    const fromDecimalPlaces = increment2digits(instrument?.[isBuy ? 'QuantityIncrement' : 'PriceIncrement'] || 0.01);
    const toDecimalPlaces = increment2digits(instrument?.[isBuy ? 'PriceIncrement' : 'QuantityIncrement'] || 0.01);
    ...
    useEffect(() => {
        if (anchored === 'from' && instrumentPrice) {
            ...
            setValue('to.amount', ((parseFloat(fromAmount) || 0) * price).toFixed(toDecimalPlaces), { shouldValidate: true });
        }
    ...
    useEffect(() => {
        if (anchored === 'to' && instrumentPrice) {
            ...
            setValue('from.amount', ((parseFloat(toAmount) || 0) * price).toFixed(fromDecimalPlaces), { shouldValidate: true });
        }
