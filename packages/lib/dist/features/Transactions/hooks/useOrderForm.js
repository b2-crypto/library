...
import { ORDER_TYPES, TIF_OPTIONS, } from '../types';
...
const schema = object({
    ...
    quantity: number()
        ...
        .when('$maxAmount', ([maxAmount], s) => maxAmount > 0 ? s.max(maxAmount, 'formError.exceedsMaxAmount') : s),
    value: number()
        .typeError(numberTypeError)
        .required(requiredErrorMessage)
        .when('op', {
...
export function useOrderForm(initialValues, context) {
    return useForm({
        defaultValues: {
            op: 'buy',
            type: 'Market',
