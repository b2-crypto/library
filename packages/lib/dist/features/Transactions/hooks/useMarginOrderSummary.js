...
export function useMarginOrderSummary(values) {
    ...
    const decimals = (isBuy ? product2?.decimalPlaces : product1?.decimalPlaces) ?? 2;
    const symbol = (isBuy ? product2?.symbol : product1?.symbol) ?? '';
    const borrowingAmount = useMemo(() => value
        ? {
            amount: roundWithPrecision(Math.max(0, value - balance), decimals).toString(),
            symbol,
        }
        : null, [value, balance, decimals, symbol]);
    //#endregion
    //# Max Buy/Sell Amount
    const { value: validationContext } = useOrderFormValidationContext();
    const maxAmount = useMemo(() => validationContext.maxAmount
        ? {
            amount: roundWithPrecision(validationContext.maxAmount, product1?.decimalPlaces ?? 2).toString(),
            symbol: product1?.symbol ?? '',
        }
        : null, [product1?.decimalPlaces, product1?.symbol, validationContext.maxAmount]);
    //#endregion
    //#region Hourly Interest
    const { data: hourlyInterestValue } = useGetAllMarginProductConfigsQuery(undefined, {
        selectFromResult: ({ data }) => ({
            data: data?.find(config => config.ProductId === (isBuy ? product2?.id : product1?.id))?.HourlyInterestRate ?? 0,
        }),
    });
    const hourlyInterest = useMemo(() => hourlyInterestValue
        ? {
            amount: roundWithPrecision(hourlyInterestValue * 100, 8) + '%',
            symbol: '',
        }
        : null, [hourlyInterestValue]);
    //#endregion
