...
import { Box, DashedBox, Image, Text, TouchableOpacity, } from '../../../components/atoms';
export const NewsItem = ({ sourceName, item }) => {
    const image = item.enclosures[0]?.url;
    return (<DashedBox bottomDash>
      <TouchableOpacity onPress={() => Linking.openURL(item.links[0]?.url)} padding="m" paddingBottom="sm">
