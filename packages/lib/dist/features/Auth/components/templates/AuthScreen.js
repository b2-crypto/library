...
export const AuthScreen = ({ logo, headerText, children, footer, }) => {
    const theme = useTheme();
    return (<ImageBackground source={loginScreenBGImage} style={[
      ...
      <SafeAreaView style={styles.flexGrowing}>
        {logo}
        <KeyboardAwareScrollView style={styles.flexGrowing} contentContainerStyle={[
            styles.loginFormWrap,
            { padding: theme.spacing.m },
        ]}>
          <Card alignSelf="stretch">
            <DashedBox bottomDash paddingVertical="m" paddingHorizontal="l">
              <Text variant="subHeadlineBold" color="brandSolid" {...testID('header')} accessible accessibilityRole="header">
                {headerText}
              </Text>
            </DashedBox>
            {children}
