...
export const SetupBiometricModal = ({ onConfirm, onDiscard }) => {
    ...
    return (<>
      ...
      <BlurBg />
      <Card variant="floating" marginTop="m" marginHorizontal="m" heading={translate('auth.biometrics.setupModal.title')} headingExtra={<Button variant="transparent" onPress={onDiscard} icon={<CloseIcon />} size="small"/>} footer={<Box>
            <Button label={translate('auth.biometrics.setupModal.button')} onPress={onConfirm} mb="s"/>
            <Button onPress={onDiscard} label={<Text variant="button_primary" color="brandSolid">
