...
export const MarginPositionCard = ({ item, onCancel, onClose, onRepay, cancelLoading, error, isProcessed = false, }) => {
    return (<Card variant="elevated" margin="m" heading={<Box flexDirection="row" alignItems="center" accessible accessibilityLabel="Title" {...testID('title')}>
          ...
          <Box flexDirection="column" ml="sm" justifyContent="center" alignItems="flex-start">
        ...
        </Box>} headingExtra={<Box width={36} height={36} alignItems="center" justifyContent="center" borderWidth={2} borderRadius={36} borderColor="border3">
          {item.PositionType === 'Long' && <PositionLong />}
          {item.PositionType === 'Short' && <PositionShort />}
        </Box>} footer={item.Status === 'Open' && (<>
            <Box flex={0} flexDirection="column">
              {!!error && (<Text color="error" textAlign="center" mb="m">
                  {error}
                </Text>)}
              {!isProcessed && (<Box flex={0} flexDirection="row" justifyContent="space-around">
                  {item.PositionState === 'Open' &&
                    item.BorrowedAmount === 0 &&
                    onCancel && (<Button loading={cancelLoading} variant="brandOpposite" label={translate('activity.marginPosition.cancelButton')} onPress={onCancel} size="big" padding="s" paddingRight="xl" paddingLeft="xl"/>)}
                  {item.PositionState === 'Open' && onClose && (<Button loading={cancelLoading} variant="brandOpposite" label={translate('activity.marginPosition.closeButton')} onPress={onClose} size="big" padding="s" paddingRight="xl" paddingLeft="xl"/>)}
                  {onRepay && (<Button loading={cancelLoading} variant="brandOpposite" label={translate('activity.marginPosition.repayButton')} onPress={onRepay} size="big" padding="s" paddingRight="xl" paddingLeft="xl"/>)}
                </Box>)}
            </Box>
          </>)}>
      <Box my="sm" mx="m">
        <DashedBox bottomDash borderColor="formBorder" py="sm" flexDirection="row" justifyContent="space-between">
          <Box width="30%">
          ...
          <Box width="25%">
            <Text variant="captionBold">
              {formatNumber(item.BorrowedAmount, item.BorrowedProductDecimalPlaces, true)}
            </Text>
            <Text variant="captionReg" color="textSecondary" mt="xxs">
        ...
        <Box py="sm" flexDirection="row" justifyContent="space-between">
          <Box width="30%">
            <Text variant="captionBold">
              {formatNumber(item.NotionalCostBasis, item.NotionalProductDecimalPlaces, true)}
            </Text>
            <Text variant="captionReg" color="textSecondary" mt="xxs">
          ...
          <Box width="40%">
            <Text variant="captionBold">
              {formatNumber(item.InterestAccrued, item.PositionProductDecimalPlaces, true)}
            </Text>
            <Text variant="captionReg" color="textSecondary" mt="xxs">
          ...
          <Box width="25%">
            {item.Status === 'Open' ? (<>
                <Text variant="captionBold" color={item.UnrealizedPnL < 0 ? 'error' : undefined}>
                ...
                <Text variant="captionBold" color={item.RealizedPnL < 0 ? 'error' : undefined}>
                  {item.RealizedPnL < 0
