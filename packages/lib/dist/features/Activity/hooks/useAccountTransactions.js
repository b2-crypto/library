...
import { useGetAccountTransactionsQuery, useLazyGetAccountTransactionsQuery, } from '../../../services/activityApi';
...
export const useAccountTransactions = (props) => {
    ...
    const { data, originalArgs, isLoading: isAccountTransactionsLoading, isUninitialized, refetch, } = useGetAccountTransactionsQuery(userInfo?.AccountId && !isInstrumentsLoading
        ? {
        ...
        selectFromResult: ({ data: res, ...rest }) => ({
            data: {
                ...
                transactions: accountTransactionsAdapter
                    .getSelectors()
                    .selectAll(res?.items ?? accountTransactionsAdapter.getInitialState())
                    .map(item => {
    ...
    const isLoading = isInstrumentsLoading || isInfoLoading || isAccountTransactionsLoading;
    const isRefetching = isLoading && !!data?.transactions?.length && !originalArgs?.offset;
