...
import { useCancelMarginPositionMutation, useCloseMarginPositionMutation, } from '../../../services/marginApi';
import { isApexError } from '../../../types/apiResponses';
import { getLogger } from '../../../services';
export const useMarginPositionActions = (item, onSuccess) => {
    const [closePosition, { isLoading, error }] = useCloseMarginPositionMutation();
    const [cancelPosition, { isLoading: isLoadingCancel, error: errorCancel }] = useCancelMarginPositionMutation();
    ...
    const onClose = useCallback(async (selectedItems) => {
        try {
            const repaymentAssetIds = selectedItems?.map(asset => asset.ProductId) || [];
            await closePosition({
