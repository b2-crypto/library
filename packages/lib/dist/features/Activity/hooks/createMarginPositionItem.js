...
export function createMarginPositionItem(item, instruments, products, notionalProduct) {
    const instrument = instruments.find(i => i.InstrumentId === item.PositionInstrumentId);
    const positionProduct = products.find(p => p.ProductId === item.PositionProductId);
    const borrowedProduct = products.find(p => p.ProductId === item.BorrowedProductId);
    return {
        ...item,
