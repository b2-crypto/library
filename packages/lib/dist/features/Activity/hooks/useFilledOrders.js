...
import { useGetAccountTradesQuery, useLazyGetAccountTradesQuery, } from '../../../services/activityApi';
...
export const useFilledOrders = (props) => {
    ...
    const { data, originalArgs, isLoading: isTradesLoading, isUninitialized, refetch, } = useGetAccountTradesQuery(currentAccount?.AccountId && !isInstrumentsLoading
        ? {
    ...
    const isRefetching = isLoading && !!data?.items?.length && !originalArgs?.offset;
