...
const transformPosition = (balance, products, marginProductConfigs, notionalProduct) => {
    const product = products?.find(p => p.ProductId === balance.ProductId);
    const productConfig = marginProductConfigs?.find(config => config.ProductId === balance.ProductId);
    return {
        ...balance,
...
export function useMarginBalances(hideSmallAmounts) {
    ...
    const { data: accountMarginBalances, refetch: refetchAccountMarginBalances } = useGetAccountMarginBalancesQuery(account ? { accountId: account.AccountId, hideSmallAmounts } : skipToken, { pollingInterval: MARGIN_REFRESH_INTERVAL });
    const transformedPositions = useMemo(() => accountMarginBalances?.map(balance => transformPosition(balance, products || [], marginProductConfigs || [], notionalProduct)), [accountMarginBalances, products, marginProductConfigs, notionalProduct]);
