...
export const MarginProductDetailsModal = ({ modalVisible, handleClose, productItem, onTransferPress, }) => {
    if (!productItem) {
        return null;
    }
    const { ProductSymbol, ProductFullName, TotalAmount, DecimalPlaces, NotionalProductSymbol, TotalAmountNotional, AvailableAmount, AvailableNotional, LiabilityAmount, LiabilityAmountNotional, OnHoldNotional, UsedMargin, UsedMarginNotional, OnHoldAmount, } = productItem;
    return (<CardModal isVisible={modalVisible} onClose={handleClose} heading={<Box flexDirection="row" alignItems="center">
      ...
      <Box padding="m" gap="m">
        <Box>
          ...
          <Box flexDirection="row" alignItems="flex-end">
            <Text variant="bodyBold" color="textPrimary" marginTop="xs" marginRight="xs">
        ...
        <Box>
          ...
          <Button variant="secondary" onPress={onTransferPress} label={translate('common.transfer')} icon={<Transfer />} marginTop="m"/>
        </Box>
      </Box>
    </CardModal>);
};
