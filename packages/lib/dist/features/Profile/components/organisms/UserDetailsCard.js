...
export const UserDetailsCard = () => {
    ...
    const onUpdatePress = async (languageCode) => {
        if (!user)
            return;
    ...
    return (<>
      <ProfileDataCard headerText={translate('common.profile')} fields={fields}/>
      <SelectLanguageModal initialValues={language.tag} modalVisible={modalVisible} isLoading={isLoading} languages={getLanguageConfig()} handleClose={hideModal} handleSubmit={onUpdatePress}/>
    </>);
};
