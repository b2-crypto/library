...
export const TwoFactorQrCard = () => {
    ...
    const qrCode = data?.ManualCode &&
        `otpauth://totp/${encodeURI(issuer + ': ' + userInfo?.UserName)}?secret=${data?.ManualCode}&issuer=${issuer}`;
    ...
    return (<Card marginTop="m" marginHorizontal="m" heading={translate('2faActivation.qrCard.title')} footer={<Button marginVertical="none" label={translate('2faActivation.qrCard.btn')} icon={<Receive color="white"/>} disabled={isLoading || !data?.ManualCode} onPress={onCopyPress}/>}>
      <Box padding="m">
        <Text>{translate('2faActivation.qrCard.text')}</Text>
        <Box padding="m" justifyContent="center" alignItems="center" borderColor="border3" borderWidth={1} borderRadius={6} marginTop="m">
          {qrCode ? (<QRCode value={qrCode} size={QR_SIZE}/>) : (<Box height={QR_SIZE} width={QR_SIZE} justifyContent="center" alignItems="center">
              <ActivityIndicator />
            </Box>)}
        </Box>
      </Box>
