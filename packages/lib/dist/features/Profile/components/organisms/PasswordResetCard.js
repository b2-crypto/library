...
export const PasswordResetCard = () => {
    ...
    return (<>
      <Card marginTop="m" marginHorizontal="m" heading={translate('profile.reset.password')} footer={<Button label={translate('common.sendEmail')} loading={isLoading} onPress={onEmailSendPress} {...testID('sendEmailButton')}/>}>
        <Box padding="m">
          <Text>{translate('profile.reset.passwordText')}</Text>
        </Box>
      </Card>

      <CardModal isVisible={modalVisible} onClose={hideModal} heading={translate('profile.reset.emailSentHeader')} footer={<Button flexDirection="column" flex={0} onPress={hideModal} label={translate('common.ok')} {...testID('emailSentOk')}/>}>
        <Box padding="m">
