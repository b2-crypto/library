...
export function useMarginInstrumentsList() {
    ...
    const { data: marginConfig, isLoading: isLoadingMargin } = useGetAllMarginInstrumentConfigsQuery();
    const data = useMemo(() => {
        if (!instruments || !products || !marginConfig) {
            return [];
        }
        return instruments.map(instrument => {
            const marginInfo = marginConfig?.find(m => m.InstrumentId === instrument.InstrumentId);
            return {
                ...instrument,
