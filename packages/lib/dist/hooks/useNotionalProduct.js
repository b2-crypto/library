...
export function useNotionalProduct() {
    ...
    const { notionalProduct } = useGetAccountPositionsQuery(userInfo?.AccountId ? { accountId: userInfo?.AccountId } : skipToken, {
    ...
    return useGetProductsQuery(undefined, {
        selectFromResult: ({ data }) => {
            if (notionalProduct && data) {
                return {
                    notionalProduct: data.find(p => p.ProductId === notionalProduct.NotionalProductId),
                };
            }
            return {};
        },
