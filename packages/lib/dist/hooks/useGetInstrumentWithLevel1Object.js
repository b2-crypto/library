...
import { useGetInstrumentsQuery, useGetLevel1SummaryQuery, useGetProductsQuery, } from '../services/apexApi';
export const useGetInstrumentWithLevel1Object = () => {
    const { data: instruments, isLoading: isLoadingInstruments } = useGetInstrumentsQuery();
        ...
        if (instruments?.length && level1?.length && products?.length) {
            instruments.forEach(instrument => {
                newData[instrument.InstrumentId] = {
                    ...
                    ProductFullName: products.find(el => el.ProductId === instrument.Product1)
