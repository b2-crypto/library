...
export function isErrorResponse(err: unknown): err is ApexErrorResponse {
  return typeof err === 'object' && err !== null && 'errormsg' in err;
}

export function isApexError(
  err: unknown,
): err is { status: number; data: unknown; message: string } {
  return (
    typeof err === 'object' &&
    err !== null &&
    'status' in err &&
...
export function isApexErrorWithApiData(
  err: unknown,
): err is { status: number; data: Record<string, unknown>; message: string } {
...
export type ResetPasswordType = { password: string } | { UserName: string };
...
type ProductLevelConfig = {
  ...
  DepositTicketWorkflow: string; // e.g. Internal
  WithdrawTicketWorkflow: string; // e.g. Internal
...
export type OpenOrderItem = {
  ...
  IsLockedIn: boolean;
  CancelReason: string;
  OrderFlag: string; // 'AddedToBook'
  UseMargin: boolean;
  StopPrice: number;
  PegPriceType: string; // 'Ask'
  PegOffset: number;
...
export type GetUserConfigRawResponse = Array<{ Key: string; Value: string }>;
...
export type ValidateUserRegistrationResponseRaw = { result: string | false };
export type ValidateUserRegistrationResponse = {
  ...
  ValidationAnswerData: {
    ...
    ProcessorData: { accessToken: string; userId: string };
...
export type WithdrawInfoResponse = Array<{
  ...
  AccountId: number;
  ProductId: number;
  ProductName: string; // it's product full name
  Name: string;
