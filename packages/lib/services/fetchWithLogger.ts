...
export const fetchWithLogger: FetchFn = async (input, init) => {
  logRequest(input);
...
function obfuscate(obj: unknown) {
...
async function logRequest(input: FetchInput) {
  let loggedRequest: string;
  if (typeof input === 'string') {
    ...
    loggedRequest = JSON.stringify({
      ...
      body:
        cloned.method === 'POST' ? obfuscate(await cloned.json()) : undefined,
    });
  }
...
async function logResponse(response: Response) {
  ...
  try {
    ...
    if (
      RESPONSE_TOO_LARGE.some(endpoint => response.url.includes(endpoint)) &&
  ...
  getLogger()(
    'debug',
    '[ Response ]: ' + JSON.stringify(logDetails),
    'fetchWithLogger',
  );
}

async function logError(error: unknown, input: FetchInput) {
  const requestUrl =
    typeof input === 'string'
      ? input
  ...
  getLogger()(
    'error',
    `[ Fetch Error ] occured for URL [${requestUrl}]: ${errorMsg}`,
    'fetchWithLogger',
  );
}
