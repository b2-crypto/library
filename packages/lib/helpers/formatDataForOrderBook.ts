...
export const formatDataForOrderBook = (
  orderBookBuy: OrderType[],
  orderBookSell: OrderType[],
  instrument: IInstrument,
) => {
  // remove duplicates with the same price
  const uniqueSell = orderBookSell.filter(
    (el, i, arr) => arr.findIndex(item => item.price === el.price) === i,
  );
  const sell = uniqueSell
    .sort((a, b) => a.price - b.price)
    .slice(0, MAX_LENGTH);

  const sellNumberOfOrdersSum: number[] = [];
  for (let i = 0; i <= sell.length - 1; i++) {
    sellNumberOfOrdersSum.push(
      sell[i].numberOfOrders + (sellNumberOfOrdersSum[i - 1] || 0),
    );
  }

  const uniqueBuy = orderBookBuy.filter(
    (el, i, arr) => arr.findIndex(item => item.price === el.price) === i,
  );

  const buy = uniqueBuy.sort((a, b) => b.price - a.price).slice(0, MAX_LENGTH);

  const buyNumberOfOrdersSum: number[] = [];
  for (let i = 0; i <= buy.length - 1; i++) {
    buyNumberOfOrdersSum.push(
      buy[i].numberOfOrders + (buyNumberOfOrdersSum[i - 1] || 0),
    );
  }
