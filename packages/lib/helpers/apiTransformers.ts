import { DataActionType, TradeHistoryItem } from '../types/apiResponses';
import { OrderType } from '../types/orderBookTypes';

export function transformFrame(frame: number[]) {
  return {
    time: frame[0],
    high: frame[1],
    low: frame[2],
...
export const transformOrder = (order: number[]): OrderType => ({
  updateId: order[0],
...
export const concatLevel2Updates = (
  draft: OrderType[],
  updates: OrderType[],
) => {
  updates.forEach(update => {
    switch (update.actionType) {
      ...
      case DataActionType.update:
        draft = draft.filter(
          listItem =>
            listItem.price !== update.price || listItem.side !== update.side,
        );
        draft.push(update);
        break;
...
export function transformTradeHistory(trade: TradeHistoryItem) {
  return {
