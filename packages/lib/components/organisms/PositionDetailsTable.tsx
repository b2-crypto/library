...
import { MarginPositionItem } from '../../features/Activity';
import { Box, Text } from '../atoms';
import { formatCrypto, translate } from '../../helpers';

type Props = {
  position: MarginPositionItem;
};

export const PositionDetailsTable = ({ position }: Props) => {
  const rows = useMemo(
    () => [
      {
      ...
      },
      {
        ...
        value: `${formatCrypto(
          ...
          position.PositionProductDecimalPlaces,
          position.Product1Symbol,
          false,
        )}`,
      },
      {
      ...
      },
      {
        ...
        value: `${formatCrypto(
          ...
          position.BorrowedProductDecimalPlaces,
          position.BorrowedProductSymbol,
          false,
        )}`,
      },
      {
        ...
        value: `${formatCrypto(
          ...
          position.NotionalProductDecimalPlaces,
          position.NotionalProductSymbol,
          false,
        )}`,
      },
      {
  ...
  return (
    <Box alignSelf="stretch" padding="s" backgroundColor="background1">
      {rows.map((item, index) => (
        <Box
          key={index}
          flexDirection="row"
