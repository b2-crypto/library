...
> & {
  ...
  listItem: React.ComponentType<{ item: DropdownListItemProps<T> }>;
...
const FullScreenContentWrapper = ({ top, children }: ContentWrapperProps) => {
...
export const DropDown = <T,>({
  ...
  const dropdownRef = useRef<RNView>();
  ...
  const renderItemOption = useCallback(
    ({ item, index }: { item: DropdownListItemProps<T>; index: number }) => (
      <Pressable
        onPress={() => {
          setValue(item.value);
          setIsOpen(false);
        ...
        {...testID((item.value as string | number).toString())}
        key={index}>
        <ListItem item={item} />
      </Pressable>
    ),
    ...
    <>
      ...
      {isOpen && !!dropdownPos && (
        <Modal visible={isOpen} transparent animationType="none">
          <Pressable
            style={styles.dropdownListBackdrop}
