...
type MarkerProps = React.ComponentProps<Required<SliderProps>['StepMarker']>;

const SliderBullet = React.memo(({ active }: { active: boolean }) => {
  return (
    <Box
      width={MARK_SIZE}
      height={MARK_SIZE}
      backgroundColor={active ? 'textSecondary' : 'formBorder'}
...
export const SliderInput = ({
  ...
  const renderMark = useCallback(
    ({ currentValue, markValue }: MarkerProps) => {
  ...
  return (
    <Box
      ...
      position="relative"
      height={displayValue ? 44 : 24}
      style={styles.container}
      onLayout={e => {
        setContainerRect(e.nativeEvent.layout);
      }}>
      {displayValue && containerRect !== undefined && (
        <Box
          position="absolute"
          left={Math.max(
            Math.min(
              containerRect.x +
                ...
                MARK_SIZE / 2,
              containerRect.width + MARK_SIZE / 2,
            ),
            -MARK_SIZE / 2,
          )}
          flexDirection="row"
      ...
      <Slider
        ...
        maximumTrackTintColor={colors.formBorder}
        value={value}
        onValueChange={onChange}
        // onSlidingComplete={() => {}}
        StepMarker={renderMark}
