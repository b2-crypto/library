...
export const bootstrap = async ({ getState, dispatch }: AppStore) => {
  const { auth } = getState();

  if (auth.user) {
    const { data: configData } = await dispatch(
      profileApi.endpoints.getUserConfig.initiate({
    ...
    const userInfo = await dispatch(
      apexApi.endpoints.getUserInfo.initiate({ userId: auth.user.UserId }),
    );

    if (userInfo.data?.AccountId) {
      await dispatch(
        apexApi.endpoints.getAccountPositions.initiate({
