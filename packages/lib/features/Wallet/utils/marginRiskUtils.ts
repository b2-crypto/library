...
export const getRiskLevel = (
  ...
  const riskMapping = RISK_LEVEL_MAPPINGS.find(
    mapping => riskNumber <= mapping.threshold,
  );
  ...
  if (riskMapping) {
    return {
      ...
      label:
        translate(
          `wallet.marginRisk.${riskMapping.label
            .toLowerCase()
            .replace(' ', '')}`,
        ) || riskMapping.label,
    };
 ...
 */
export const getActiveSegment = (risk: number) => {
 ...
 */
export const getRiskColorKey = (risk: number) => {
  if (risk <= RISK_THRESHOLDS.VERY_LOW) {
    return 'riskVeryLow';
  }
  if (risk <= RISK_THRESHOLDS.LOW) {
