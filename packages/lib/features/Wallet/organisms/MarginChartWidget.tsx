...
export const MarginChartWidget = () => {
  const [accountDetailsModalOpen, toggleAccountDetailsModal] = useReducer(
    v => !v,
    false,
  );
  ...
  const riskLevel = getRiskLevel(
    marginAccountInfo?.RiskLevel ?? 0,
    marginAccountInfo?.InMarginCall ?? false,
    marginAccountInfo?.InLiquidation ?? false,
  );

  return (
    <Box p="m" alignItems="center">
      <MarginAccountDetailsModal
        modalVisible={accountDetailsModalOpen}
        handleClose={toggleAccountDetailsModal}
