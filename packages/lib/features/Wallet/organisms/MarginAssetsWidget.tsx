...
import {
  MarginWalletItem,
  MarginProductDetailsModal,
  MarginWalletItemType,
} from '../molecules';
...
export const MarginAssetsWidget = () => {
  ...
  const [hideSmallAmountsValue, setHideSmallAmountsValue] = useStringValue(
    STORAGE_KEYS.HIDE_MARGIN_SMALL_AMOUNTS,
    getDefaultStorage(),
  );
  const hideSmallAmounts = hideSmallAmountsValue === 'true';
  const { data: collateralFunds, refetch } =
    useMarginBalances(hideSmallAmounts);

  const handleProductPress = (item: MarginWalletItemType) => {
  ...
  const filteredCollateralFunds = useMemo(() => {
    return (
      collateralFunds?.filter(
        p =>
          p.CollateralEnabled &&
          (p.ProductFullName.toLowerCase().includes(
  ...
  const handleTransferPress = () => {
    ...
    const productId =
      ...
      collateralFunds?.reduce(
        (
          highest: MarginWalletItemType | undefined,
          current: MarginWalletItemType,
        ) =>
          !highest || current.ProductId < highest.ProductId ? current : highest,
        undefined as MarginWalletItemType | undefined,
      )?.ProductId;
  ...
  return (
    <Box flex={1}>
      ...
      <FlatList
        ...
        keyExtractor={item => item.ProductFullName}
