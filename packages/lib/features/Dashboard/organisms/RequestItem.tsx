...
export const RequestItem = ({ request }: { request: RequestModel }) => {
  const { submitting, onSubmit, error } = useUserRequestConfirm();
  const [type, setType] = useState<string | null>();
  return (
    <ExpandingCard expandableContent={<RequestDetails request={request} />}>
      <DashedBox padding="m" flexDirection="row" bottomDash>
        ...
        <Box marginLeft="sm">
          ...
          {error && isApexError(error) && (
            <Text textAlign="center" variant="bodyBold" color="error" mt="xs">
              {error.message}
            </Text>
          )}
        </Box>
      ...
      <ModalBlurBg
        isVisible={!!type}
        onBackdropPress={() => setType(null)}
        style={styles.modal}>
        {!!type && (
          <TransferRequestConfirm
            request={request}
            type={type}
