...
export const TroubleLoggingForm = ({
  resendError,
  resetError,
  isLoading,
  onSubmit,
}: TroubleLoggingFormProps) => {
  const { watch, handleSubmit, control, clearErrors, setFocus, setValue } =
    useForm<FormValues>({
  ...
  useEffect(
    () => setFocus(issueType === 'userName' ? 'userName' : 'email'),
    [issueType],
  );
    ...
    <>
      <Box padding="m" borderBottomWidth={1} borderColor="border3">
        ...
        <Box mt="sm">
          <RadioGroup
            ...
            value={issueType}
            onChange={v => setValue('issueType', v)}
          />
        </Box>
      </Box>
      ...
      <Box
        ...
        <Button
          ...
          loading={isLoading}
          variant="primary"
          label={translate('auth.loginScreen.sendEmail')}
