...
type TablesTabs = 'orderBook' | 'recentTrades';

export const PairTabsTables = ({
  orderBook,
  spread,
  trades,
}: PairTabsProps) => {
  const [activeTab, setActiveTab] = useState<TablesTabs>('orderBook');

  const tabButtons = useMemo(
    () =>
      (['orderBook', 'recentTrades'] as TablesTabs[]).map(tab => ({
        value: tab,
        label: translate(tab),
      })),
    [],
  ...
  return (
    <>
      ...
      {activeTab === 'orderBook' ? (
        <OrderBookTable data={orderBook} spread={spread} />
      ) : (
...
export const PairTabsTablesWidget = memo(
  ({ instrumentId, skipQuery = false }: PairTabsTablesWidgetProps) => {
    ...
    const spread = useMemo(() => {
      ...
      return formatNumber(
        latestRow?.[1].rawValue - latestRow?.[0].rawValue || 0,
        increment2digits(instrument?.PriceIncrement || 2),
        true,
      );
