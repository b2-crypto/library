...
export function useMarginPrechecks() {
  ...
  const { data: accountInfo, isLoading: accountLoading } =
  ...
  const { data: kvpValues, isLoading: kvpLoading } = useGetMarginKVPQuery(
    isMarginEnabled ? skipToken : undefined,
  );
  const { data: positions, isLoading: positionsLoading } =
    useGetAccountPositionsQuery(
      userInfo ? { accountId: userInfo.AccountId } : skipToken,
    );
  const totalAccountValue =
    positions?.reduce((sum, p) => sum + p.NotionalValue, 0) || 0;
  ...
  if (kvpValues && accountInfo && !isMarginEnabled) {
    ...
    data.isVLEligible = marginVerificationLevels.includes(
      accountInfo.VerificationLevel,
    );
