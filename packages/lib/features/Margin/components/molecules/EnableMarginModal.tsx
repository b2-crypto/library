...
type ModalTypes = 'viewTerms' | 'acceptTerms' | 'decline' | 'error' | 'success';

export function EnableMarginModal({
  isEnabled,
  onEnable,
  loading,
  error,
  ...
  const [visibleModal, setVisibleModal] = React.useState<ModalTypes | null>(
    null,
  );
  ...
  const onModalClose = React.useCallback(
    () =>
      setVisibleModal(active => (active === 'acceptTerms' ? 'decline' : null)),
    [setVisibleModal],
  );
  ...
  if (visibleModal === 'acceptTerms') {
    modalButtons = (
      <Box flexDirection="row" justifyContent="space-between" gap="m">
        <Button
          flex={1}
          label={translate('profile.margin.confirmButton')}
          disabled={!termsAccepted}
          loading={loading}
          onPress={handleEnableMargin}
          {...testID('marginAcceptTermsButton')}
        />
        <Button
          flex={1}
          variant="line"
          label={translate('profile.margin.declineButton')}
          onPress={() => setVisibleModal('decline')}
          {...testID('marginDeclineTermsButton')}
        />
      </Box>
    );
  } else if (
    visibleModal === 'success' ||
    visibleModal === 'error' ||
    ...
    modalButtons = (
      <Button
        ...
        flex={0}
        variant={visibleModal === 'success' ? 'primary' : 'secondary'}
        label={translate('common.ok')}
        onPress={() => setVisibleModal(null)}
        {...testID('marginTermsOkButton')}
      />
    );
  }
      ...
      <>
        ...
        <Box mx="m" mb="m">
          <Checkbox
            ...
            label={translate('profile.margin.termsCheckboxLabel')}
            onChange={() => setTermsState(!termsAccepted)}
          />
        </Box>
      </>
    );
  ...
  return (
    <>
      <Card m="m" p="m">
        {isEnabled ? (
          <Pressable onPress={() => setVisibleModal('viewTerms')}>
          ...
          <Button
            label={translate('profile.margin.applyButton')}
            onPress={() => setVisibleModal('acceptTerms')}
          />
        )}
      </Card>
      <CardModal
        ...
        kind={
          visibleModal === 'error' || visibleModal === 'decline'
            ? 'error'
            : 'default'
        }
        heading={getModalHeader()}
        footer={modalButtons}>
        {modalBody}
      </CardModal>
    </>
