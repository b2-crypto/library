...
import { KYCLevelData, VerificationLevelSection } from '../organisms';
  ...
  <Box>
    <ItemWithBg>
      ...
      <Text
        ...
        marginTop="xs"
        accessible
        accessibilityLabel="Current verification level"
        {...testID('currentLevel')}>
    ...
    <ScrollView contentContainerStyle={styles.pageWrapper}>
      {visibleLevels.map((level, idx) => (
        <Box
          key={level.level}
          mb={idx + 1 !== visibleLevels.length ? 'm' : undefined}>
...
export const VerificationTabWidget = () => {
  ...
  const visibleLevels = useMemo(
    () =>
      (container.getConfigValue('verificationLevels') ?? []).map(level => ({
        ...level,
