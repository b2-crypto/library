...
import { SEND_TYPE, SendFormValues, SendType } from '../types';
import {
  emailError,
  numberTypeError,
  positiveNumberError,
...
const schema = object({
  ...
  templateForm: mixed().when(
    ['type', 'productId', '$productSymbols'],
    ([type, productId, symbols], s) => {
      if (type !== 'withdraw') {
        return s;
      }
      return lazy(obj => {
          ...
          if (key === 'ExternalAddress') {
            return string()
              ...
              .test(
                'validate-crypto',
                () =>
                  translate('formError.invalidCryptoAddress', {
                    symbol: symbols[productId],
                  }),
                value =>
                  !value || validateCryptoAddress(value, symbols[productId]),
              );
          }
