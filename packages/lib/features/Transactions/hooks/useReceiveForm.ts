...
import { ReceiveFormValues, ReceiveType } from '../types';
import {
  emailError,
  numberTypeError,
...
const schema = object({
  ...
  emailAddress: string()
    ...
    .when('$userEmail', ([userEmail], s) =>
      userEmail
        ? s.notOneOf<string>([userEmail], 'formError.notPersonalEmail')
        : s,
    ),
  amount: number()
    .positive()
...
export function useReceiveForm(
  ...
  return useForm<ReceiveFormValues>({
