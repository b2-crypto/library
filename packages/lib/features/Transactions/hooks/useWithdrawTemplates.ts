...
export function useWithrawTemplates(productId?: number, providerId?: number) {
  const { data: userInfo } = useUserInfo();

  const { data: templatesData, isLoading: templatesLoading } =
    useGetWithdrawTemplateTypesQuery(
      userInfo?.AccountId && productId
        ? { AccountId: userInfo.AccountId, ProductId: productId }
        : skipToken,
    );

  const templates = useMemo(
    () => templatesData?.TemplateTypes || [],
    [templatesData],
  );
  ...
  const {
    ...
    originalArgs: templateInfoArgs,
  } = useGetWithdrawTemplateInfoQuery(
    productId && templateType ? { productId, ...templateType } : skipToken,
  );

  const { productConfig } = useGetVerificationLevelConfigQuery(
  ...
  const { allowedAddressOptions } = useGetAccountWithdrawInfosQuery(
    userInfo?.AccountId && productConfig?.RequireWhitelistedAddress
  ...
  return {
    templates,
    templateInfo:
      templateInfoArgs?.AccountProviderId === providerId &&
