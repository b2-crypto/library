...
type ConfirmModalProps = {
  ...
  data: Array<{ label: string; value: string; vertical?: boolean }>;
  onClose: () => void;
...
const getOrderedData = (
  data: { label: string; value: string; vertical?: boolean }[],
): { label: string; value: string; vertical?: boolean }[] => {
...
export const ConfirmView = ({
  ...
  return (
    <Card variant="elevated">
      ...
      <Box paddingHorizontal="m" role="list">
        {orderedData.map(item => (
          <DashedBox
            ...
            alignItems={item.vertical ? 'flex-start' : 'center'}
            justifyContent={item.vertical ? 'flex-start' : 'space-between'}
            flexWrap="wrap"
            accessible
            accessibilityLabel={`${item.label} ${item.value}`}
            {...testID(`confirmItem.${item.label}`)}>
            <Text
              variant="captionReg"
              color="textSecondary"
              mr={!item.vertical ? 'xs' : undefined}
              mb={item.vertical ? 'xs' : undefined}>
              {item.label}
            </Text>
            <Text
              variant="bodyReg"
              textAlign={item.vertical ? 'left' : 'right'}>
              {item.value}
            </Text>
          </DashedBox>
        ))}
      ...
      {!!button && (
        <Box padding="m" borderTopColor="border3" borderTopWidth={1} mt="m">
