...
export const OrderFormBody = ({
  ...
  return (
    <>
      {product1 && product2 ? (
        <Box flexDirection="column" padding="m" gap="sm">
          {(orderType === 'StopMarket' || orderType === 'StopLimit') && (
            <FormAmountInput
              control={control}
              symbol={product2.symbol}
              decimalPlaces={product2.decimalPlaces}
              name="stopPrice"
              label={translate('transaction.fields.stopPrice')}
              placeholder="0"
          ...
          <Box flexDirection="row" gap="m" paddingBottom="xs">
            <Box flexGrow={1} px="s">
              <AmountSlider
                maxQuantity={maxQuantity}
                decimalPlaces={product1.decimalPlaces}
              />
            </Box>
        ...
        <Box height={100} justifyContent="center" alignItems="center" m="xl">
          <Text
            textAlign="center"
            accessible
            accessibilityLabel="Select assect"
            {...testID('selectAsset')}>
