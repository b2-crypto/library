...
const is2FAError = (err: unknown) =>
  isApexError(err) && err.message === 'Waiting for 2FA';

export const DepositOrWithdrawForm = ({
  productId,
  onClose,
  ...
  const values = getValues() as DepositFormValues & { type: 'send' };
  ...
  const onSubmit = useCallback(
    async (payload: DepositFormValues) => {
      try {
        await send({ values: payload, assetId });
        Toast.show({
          type: CustomToast.text,
          text1: translate(
            `transaction.${typeTransaction}.${payload.type}Success`,
          ),
        });
        onClose();
      } catch (err) {
    ...
    <FormProvider {...form}>
      <ScrollView keyboardShouldPersistTaps="always">
        <DepositFormHeader onClose={onClose} typeTransaction={typeTransaction}>
          <AssetField
            assets={assets}
            label={translate(`${typeTransaction}`)}
        ...
        {assetId && asset ? (
          <>
            ...
            <Box paddingHorizontal="m" marginVertical="m">
              {type === 'external' ? (
                <DepositExternalFormWidget asset={asset} />
              ) : (
                <DepositRequestFormWidget asset={asset} />
              )}
            </Box>
          </>
        ) : (
          <Box padding="m">
            <SelectAssetMessage
              message={translate('transactions.selectAsset.send')}
            />
          </Box>
        )}
        {!!error && isApexError(error) && !is2FAError(error) && (
          <Box mt="m">
        ...
        <Box padding="m" mt="m">
          <Button
            ...
            label={translate(`transaction.${typeTransaction}.requestButton`)}
            onPress={showModal}
            {...testID('submit')}
          />
        </Box>
      ...
      <ModalBlurBg
        ...
        style={styles.modal}>
        {!!error && is2FAError(error) ? (
          <Transfer2FA onSuccess={onClose} onClose={hideModal} />
        ) : (
          modalShown && (
            <DepositOrWithdrawConfirm
              values={values}
              asset={asset}
              onClose={hideModal}
