...
const createProductIcon = (symbol: string, disabled: boolean) => {
  const Icon = () => (
    <ProductIcon symbol={symbol} size={24} disabled={disabled} />
  );
  return Icon;
};

const getFormattedAmountForDisplay = (
  amount: number,
  decimalPlaces: number,
) => {
...
export const ProductAmountField = memo(
  ({
    assetFieldName,
    amountFieldName,
    label,
    ...
    const productId: number | undefined = watch(assetFieldName);
    const { field: amountField } = useController<{ [k: string]: string }>({
    ...
    const footerContent = useMemo(() => {
      const formattedAmount = getFormattedAmountForDisplay(
        product?.amount || 0,
      ...
      <BoxFormItem
        ...
        labelExtra={
          showNotionalValue && (
            <Text
              variant="bodyReg"
              accessible
              accessibilityLabel={`${label} — notinal amount`}
              {...testID(`${label}NotionalAmount`)}>
              {product?.notionalRate &&
              product?.notionalSymbol &&
              amountField.value
                ? formatCurrency(
                    product.notionalRate * (parseFloat(amountField.value) || 0),
                    product.notionalSymbol,
                    2,
                  )
                : '—'}
        ...
        <Box width="100%" flexDirection="row">
          ...
          <FormAmountInput
            control={control}
            name={amountFieldName}
            decimalPlaces={decimalPlaces || product?.decimalPlaces}
            onFocus={onFocus}
            defaultValue={Number(0).toFixed(product?.decimalPlaces || 2)}
            maxLength={
              ignoreMaxLength
                ? undefined
                : isFocused
            ...
            suffix={
              suffix ||
              (showMaxButton && !!product?.amount && (
                <MaxButton
                  onPress={() => {
                    if (product.amount) {
                      const maxValue = roundWithPrecision(
                        product.amount,
                        product.decimalPlaces,
                      );
                      setValue(
                        amountFieldName,
                        maxValue.toFixed(product.decimalPlaces),
                      );
