  ...
  const filters = useMemo(() => {
    return Object.values(
      instruments.reduce((acc, i) => {
        ...
        return acc;
      }, {} as Record<number, TabItem<number>>),
    );
  }, [instruments]);

  const options = useMemo(() => {
    const filteredOptions: InstrumentOption[] = instruments
      ...
      .map(el => ({
        ...
        icon: () => (
          <ProductPairIcon
            size={24}
            symbol1={el.Product1Symbol}
            symbol2={el.Product2Symbol}
          />
        ),
      }));
      ...
      if (
        ...
        filteredOptions.push({
          ...
          icon: () => (
            <ProductPairIcon
              size={24}
              symbol1={selectedInstrument.Product1Symbol}
              symbol2={selectedInstrument.Product2Symbol}
            />
          ),
