...
export const useClosedMarginPositions = (props: {
  ...
  const { data: marginAccount, isLoading: isLoadingAccount } =
    useMarginAccount();
  ...
  const {
    ...
    marginAccount?.AccountId && !isInstrumentsLoading
      ? {
  ...
  const isLoading =
    isPositionsLoading || isLoadingAccount || isInstrumentsLoading;
  const isRefetching =
    isLoading && !!data?.positions?.length && !originalArgs?.offset;
