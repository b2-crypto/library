...
const selector = createSelector(
  ...
  (_: RootState, accs: Array<{ AccountId: number }> | undefined) =>
    accs?.map(a => a.AccountId) ?? [],
  (state: RootState, accountIds: number[]) =>
    Object.fromEntries(
...
export function useAllAccountsPositions() {
  ...
  useEffect(() => {
    ...
    const subscriptions = accounts.map(account =>
      dispatch(query.initiate({ accountId: account.AccountId })),
    );
    return () => subscriptions.forEach(s => s.unsubscribe());
  }, [accounts, dispatch]);

  return useAppSelector<Record<number, IAccountPosition[]>>(state =>
    selector(state, accounts),
  );
}
