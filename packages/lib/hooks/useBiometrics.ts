...
export const useBiometrics = () => {
  ...
  const [isBiometricsEnabled, setBiometricEnabled] = useBoolValue(
    ENCRYPTED_STORAGE_KEYS.IS_BIOMETRIC_ENABLED,
    encryptedStorage,
  );
  ...
  const promptBiometrics = useCallback(async () => {
    try {
      await rnBiometrics.requestBioAuth(
        translate('auth.biometrics.title'),
        translate('auth.biometrics.confirm'),
      );
      return true;
    } catch (error) {
